-- ============================================================================
-- ALINHAMENTO MOCK-UP × QUERY ORIGINAL — BRIEFING VISITA MINISTERIAL
-- ============================================================================
-- Documento de alinhamento para o time de desenvolvimento Data Studio /
-- Looker Studio. A query abaixo é a query ORIGINAL fornecida (query.sql),
-- preservada sem alterações. Este cabeçalho apenas mapeia cada NÚMERO
-- visível em mock-up.png à coluna correspondente do SELECT FINAL.
--
-- ABA DO MOCK-UP COBERTA POR ESTE ALINHAMENTO: "Educação Básica"
-- GRANULARIDADE: por UF (27 estados). A coluna `uf` é a chave do filtro Estado.
-- O Brasil agregado vem CONCATENADO dentro de cada string ("UF | Brasil: ...").
--
-- ┌────┬────────────────────────────────────────────┬────────────────────────────────────┬─────────────────────┐
-- │ #  │ Métrica no mock-up                         │ Coluna do SELECT FINAL             │ CTE de origem        │
-- ├────┼────────────────────────────────────────────┼────────────────────────────────────┼─────────────────────┤
-- │ 1  │ Escolas Conectadas Nível 4 e 5             │ escolas_conectadas_nivel_4_5       │ escola_conectada     │
-- │ 2  │ ETI – Matrículas Fomentadas (2023/2025)    │ eti_matriculas_fomentadas          │ escola_eti           │
-- │ 3  │ ETI – Valor Fomentado                      │ eti_valor_fomentado                │ escola_eti           │
-- │ 4  │ CNCA – Cantinho de Leitura                 │ cnca_cantinho_leitura              │ cnca                 │
-- │ 5  │ CNCA – Escolas Apoiadas                    │ cnca_escolas_apoiadas              │ cnca                 │
-- │ 6  │ CNCA – Valor Investido em Cantinhos        │ cnca_valor_cantinhos               │ cnca                 │
-- │ 7  │ CNCA – Articuladores RENALFA 2025          │ cnca_qtd_articuladores_total       │ cnca                 │
-- │ 8  │ CNCA – Repasse para Articuladores RENALFA  │ cnca_articuladores_renalpha_2026   │ cnca                 │
-- │ 9  │ CNCA – Empenho Aquisição de Materiais      │ cnca_valor_materiais               │ cnca                 │
-- │ 10 │ CNCA – Empenho Formação Profissional       │ cnca_valor_formacao                │ cnca                 │
-- │ 11 │ CNCA – Total Investido                     │ cnca_total_investido               │ cnca                 │
-- │ 12 │ Pé-de-Meia – Estudantes Beneficiados (+ %) │ pdm_estudantes_beneficiados        │ pdm                  │
-- │ 13 │ Pé-de-Meia – Valor Investido               │ pdm_valor_investido                │ pdm                  │
-- │ 14 │ Taxa de Distorção Idade-Série 2023→2025    │ distorcao_idade_serie              │ distorcao_serie      │
-- └────┴────────────────────────────────────────────┴────────────────────────────────────┴─────────────────────┘
--
-- LINHAS NA QUERY ORIGINAL (para navegação no editor):
--   #1  escola_conectada       → linha 137
--   #2  escola_eti             → linha 170 (coluna escola_eti_qtd_matricula)
--   #3  escola_eti             → linha 170 (coluna escola_eti_valor_fomento)
--   #4-#11  cnca                → linha 788 (deps: linhas 685-849)
--   #12 pdm                    → linha 288 (coluna pdm_estudantes_beneficiados)
--   #13 pdm                    → linha 288 (coluna pdm_valor_investido)
--   #14 distorcao_serie        → linha 962
--   SELECT FINAL               → linha 1065
--
-- ============================================================================
-- CAMPOS DO HEADER DO MOCK-UP NÃO COBERTOS POR ESTA QUERY
-- ============================================================================
-- O card superior do mock-up exibe dois números que NÃO estão nesta query:
--   • "212,58 mi População (IBGE)" — fonte sugerida: IBGE / dados_mestres
--   • "5,7 IDEB Anos Iniciais (2023)" — fonte sugerida: INEP IDEB
-- Esses campos precisam de fonte adicional no Data Studio (data source separado
-- ou union com a presente query). Não estão no escopo do alinhamento atual.
--
-- ============================================================================
-- COMO USAR NO DATA STUDIO / LOOKER STUDIO
-- ============================================================================
-- 1. Conectar BigQuery → "Custom Query" → projeto: br-mec-segape-dev
--    (todos os FROM da query usam dataset qualificado, não precisa default).
-- 2. Colar TODO o conteúdo abaixo desta linha tracejada como a Custom Query.
-- 3. Resultado: 27 linhas (uma por UF). Coluna `uf` = sigla, `territorio` = nome.
-- 4. Para cada cartão numerado do mock-up, vincular a coluna apontada na tabela
--    acima. Os valores já vêm formatados como string ("UF | Brasil: ...") —
--    se quiser separar UF e Brasil em campos distintos, fazer SPLIT por " | ".
-- 5. NÃO USAR a parte "ANEXO SESU" / "ANEXO SETEC" da query original
--    (linhas 1115-1124): são SELECTs auxiliares para a aba Educação Superior,
--    fora do escopo deste mock-up.
--
-- ============================================================================
-- AVISOS IMPORTANTES (extraídos das auditorias da query original)
-- ============================================================================
-- • HARDCODES BRASIL preservados intencionalmente para alinhar com o briefing:
--     – #3   eti_valor_fomentado Brasil:        'R$ 7,16 bilhões'
--     – #7   cnca_qtd_articuladores Brasil:     '7.386'
--     – #11  cnca_total_investido Brasil:       'R$ 1,56 bilhão'
--   Detalhes nos blocos [AUDITORIA] dentro do corpo da query (linhas 685-787).
--
-- • FILTRO MUNICÍPIO do mock-up (coluna "Município" das tabelas):
--   ESTA QUERY NÃO TEM granularidade de município. Os valores na coluna
--   "Município" do mock-up exigem uma query municipal separada (não fornecida
--   neste pacote). O Data Studio precisará de uma segunda fonte para essa coluna.
--
-- • PERCENTUAIS entre parênteses (itens #1 e #12):
--   Já vêm embutidos na string. Para usar como número, será necessário
--   parseá-los com REGEXP_EXTRACT no Data Studio ou criar uma view adicional.
--
-- ============================================================================
-- A PARTIR DESTA LINHA SEGUE A QUERY ORIGINAL — NÃO ALTERAR
-- ============================================================================
