...
**Regra crítica**: listar os modelos explicitamente com `--select`. **Nunca** usar o prefixo `+` no seletor para modelos de `projeto_painel_ministro` — isso re-materializa todos os modelos upstream e pode destruir policy tags IAM.
...
Misturar escopos dificulta a revisão e pode atrasar a aprovação.
